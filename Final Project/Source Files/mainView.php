	  ...
	  <script>
		...
		function loadLocations() {
			...
			if(selectedVal) {
				...
				{
					curChars += "<form action=\"moveLoc.php\" method=\"get\">"
								 + "<input type=\"hidden\" name=\"charName\" value=\"" + locChars[selectedVal][i] + "\">"
							     + locChars[selectedVal][i] + "  </input><input type=\"submit\" value=\"Move\">"
								 + "<span>  to  </span>" + createLocs(selectedVal) + "</form>";
				}

				curChars += "<p>Type: " + locType[locData[selectedVal].Type] + "</p>"
						  + "<p>Location: " + 	locData[selectedVal].X_Coordinates + ", " + locData[selectedVal].Y_Coordinates + "</p>";
		...
		function createLocs( except ) {
			returnHTML = "<select Name=\"locName\">";
			for (var locName in locData) {
				if(locData.hasOwnProperty(locName)) {
					if(locName != except){
						returnHTML += "<option value=\"" + locName + "\">" + locName + "</option>";
					}
				}
			}
			returnHTML += "</select>";
